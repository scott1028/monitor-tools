...
// testcase-1
const RounteCmp = ({onClick, version, dispatch}) => {
  return (
...
const RounteCmp2 = connect(state => {
  return {
    version: state.version,
  }
})(RounteCmp);

const NoLoggedInDom = ({version, dispatch}) => {
  return (
    <div onClick={() => {
      dispatch({
        type: 'ADD',
      ...
      DEFAULT.value += 3;
      console.log(DEFAULT.value);
    }}>No LoggedIn: {version}</div>
  )
}

const NoLoggedInDom2 = connect(state => {
  return {
    version: state.version,
  }
})(NoLoggedInDom);
